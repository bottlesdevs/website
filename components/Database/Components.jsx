...
const Components = () => {
    ...
    useEffect(() => {
        const fetchComponents = async () => {
            try {
                const response = await fetch('https://raw.githubusercontent.com/bottlesdevs/components/main/index.yml');
        ...
        <div className="min-h-screen bg-zinc-50 dark:bg-black transition-colors duration-300">
            <section className="pt-32 pb-20 border-b border-zinc-200 dark:border-white/5 bg-white dark:bg-zinc-900/20">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <h1 className="text-5xl font-bold text-zinc-900 dark:text-white mb-6 tracking-tight">{t.database.components.title}</h1>
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
                    {loading ? (
                        <div className="p-12 text-center">
                            ...
                            <p className="text-zinc-500">{t.database.components.loading}</p>
                        </div>
                    ) : (
                            ...
                            <table className="w-full text-left">
                                <thead className="bg-zinc-50 dark:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-800">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 font-semibold text-zinc-900 dark:text-white">{t.database.table.type}</th>
                                        <th className="px-6 py-4 font-semibold text-zinc-900 dark:text-white">{t.database.table.category}</th>
                                        <th className="px-6 py-4 font-semibold text-zinc-900 dark:text-white">{t.database.table.channel}</th>
                                        ...
                                        <tr key={name} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <div className="flex gap-4 text-sm">
                                                    {(name.toLowerCase().includes('caffe') || name.toLowerCase().includes('soda')) ? (
                                                        <a
                                                            href="/runners"
                                                            className="text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1"
                                                        >
                                                            {t.database.table.learnMore} <ExternalLink className="w-3 h-3" />
                                                        </a>
                                                    ) : (
                                                        <a
                                                            href={`https://github.com/bottlesdevs/components/blob/main/${data.Category}/${name}.yml`}
                                                            target="_blank"
                                                    ...
                                                    <a
                                                        href="https://github.com/bottlesdevs/components/issues/new/choose"
                                                        target="_blank"
                                                        rel="noopener noreferrer"
